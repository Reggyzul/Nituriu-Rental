...
export default function TransportRentPage({ onSelectCar, lang, onNavigateHome }: TransportRentPageProps) {
  ...
  const handleWhatsAppBooking = (car: Car) => {
    ...
    const message = isEN
      ? `Hello Nituriu Travel, I am interested in renting: ${car.name}. Please inform price quote, schedule & unit availability. Thank you!`
      : `Halo Nituriu Travel, saya berminat sewa armada: ${car.name}. Mohon informasi penawaran harga, jadwal & ketersediaan unit. Terima kasih!`;
